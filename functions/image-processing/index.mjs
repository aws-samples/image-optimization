...
export const handler = async (event) => {
    ...
    try {
        ...
        var resizingOptions = {}
        const dpr = operationsJSON['dpr'] ? parseFloat(operationsJSON['dpr']) : 1
        if (operationsJSON['width']) resizingOptions.width = Math.round(parseInt(operationsJSON['width']) * dpr)
        if (operationsJSON['height']) resizingOptions.height = Math.round(parseInt(operationsJSON['height']) * dpr)
        if (operationsJSON['fit']) {
            resizingOptions.fit = operationsJSON['fit'] // 'contain' | 'cover' - 'cover' is the default
        }
        ...
        if (operationsJSON['format']) {
            ...
            if (operationsJSON['quality'] && operationsJSON['format'] && isLossy) {
                transformedImage = transformedImage.toFormat(operationsJSON['format'], {
                    quality: parseInt(operationsJSON['quality']),
                })
            } else if (operationsJSON['format']) transformedImage = transformedImage.toFormat(operationsJSON['format'])
        } else {
